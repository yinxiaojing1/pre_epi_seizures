...
class EAC_CSR():
    ...
    def _allocate_arrays(self):
        ...
        self.data = np.empty(total_assocs, dtype=self.dtype)
    ...
    def _validate_func_options(self):
        ...
        if self.sort_mode not in sort_options:
            raise ValueError("sort_mode must be one of the following: " + 
                            str(sort_options))

        max_assocs_options = ("constant","linear")
        if self.max_assocs_type not in max_assocs_options:
            raise ValueError("max_assocs_type must be one of the following: " + 
                            str(max_assocs_options))

        if self.max_assocs_type != "constant" and not self.condensed:
            errmsg = "max_assocs_type must be \"constant\" if matrix is condensed"
    ...
    def _update_partition_numpy(self, partition):
        for cluster in partition:
            new_nnz = self.normal_fn(self.indices, self.data, self.indptr,
...
def indptr_constant(n_samples, max_assocs):
    return np.arange(n_samples + 1, dtype=np.int32) * max_assocs
