...
# Bioplux device class
class BioPluxDevice(object):
    def __init__(self, mac='test', SamplingRate=1000.):
        self.SamplingRate = SamplingRate
        try:
            self.device = self.connect(mac)['BioPluxDevice']
        except Exception as e:
            print e
            self.device = None
    # Connect to Bioplux device
    def connect(self, mac='test'):
        """
    ...
    def start(self):
        ...
        self.device.beginAcq(int(self.SamplingRate), 0xFF, 12)
        return {'BioPluxDevice': self.device, 'SamplingRate': self.SamplingRate}
    # Get frames
    def frames(self, nsamples=100):
        """
        ...
        frames = self.device.getFrames(nsamples)
        hdr = {'Version': 0, 'SamplingResolution': 12., 'SamplingFrequency': float(self.SamplingRate), 'Vcc': 5., 'Units': 'ADC'}
